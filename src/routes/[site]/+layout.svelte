<script>
	...
	import Wdt, { database_subscribe, storage_subscribe, deploy_subscribe } from '@worlddev/cms-builder'
	...
	deploy_subscribe(async (untyped_payload, create_new) => {
		...
		/** @type {import('@worlddev/cms-builder/src/lib/deploy.js').DeploymentPayload} */
		...
		if (payload.provider === 'github') {
			const chunks = chunk_payload(payload, 3000) // break payload up into chunks to avoid cloud function body limits
			const blob_list = []
			for (let key in chunks) {
				blob_list[key] = await create_blob_list(chunks[i])
			}
			return await deploy_to_server({
				...payload,
				files: blob_list.flat(),
				message: `Deploy site`
			})
