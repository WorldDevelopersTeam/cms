...
const github = {
   ...
   * @returns {Promise<import('@worlddev/cms-builder/src/lib/deploy.js').DeploymentResponse>}
   */
  push_site: async function ({ repo_name, files, message, token }) {
    ...
    async function create_tree() {
      const tree = files.filter(file => file !== null).map((file) => ({
        path: file.path,
        sha: file.sha,
        type: 'blob',
        mode: '100644',
 ...
 * @property {string} repo_name - The name of the repository where the files will be stored.
 * @property {Array<import('@worlddev/cms-builder/src/lib/deploy.js').File>} files - The files to be uploaded to the repo
